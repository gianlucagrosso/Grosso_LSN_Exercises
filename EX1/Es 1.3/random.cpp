...
// rejection method sampling
   ...
   while((pow(x,2)+pow(y,2)) >= 1);  // exit from the cycle when the point (x, y) falls inside the semicircle witn radius 1

   return asin(y/(sqrt(pow(x,2)+pow(y,2)))); // return the polar angle Theta
}
