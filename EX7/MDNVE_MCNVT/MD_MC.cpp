...
{ 
  ...
  if(restart==0 and iEQ == 0) Equilibration(); //Equilibration
...
//Read input informations
  ...
  if(iNVET) path = "DATA/MC/";
  else path = "DATA/MD/";


  ReadInput >> restart;
  ReadInput >> iEQ; //variable to determine whether we are doing equilibration or not
  ...
  vtail = (8.0*pi*rho)*(1.0/(9.0*pow(rcut,9)) - 1.0/(3.0*pow(rcut,3)));
  ptail = (32.0*pi*rho*npart)*(1.0/(9.0*pow(rcut,9)) - 1.0/(6.0*pow(rcut,3)))/vol;
  ...
  cout << "Bins for g(r) histogram: " << nbins << endl;
...
//Prepare arrays for measurements
  ...
  ie = 3; //Total energy
  ip = 4; //Pressure
  ig = 5; //Radial distribution
  n_props = 6; //Number of observables

  for(int i = 0; i<nbins; i++) //Reset the walker for the radial distribution
      walker[ig+i] = 0;
...
//Read initial configuration
  ...
  if(restart)
  {
    ReadConf.open(path_out + phase + "_config.out");
    ReadVelocity.open(path_out + phase + "_velocity.out");
...
//Print initial values for measured properties
  ...
  cout << "Pressure                 (with tail corrections) = " << walker[ip] + ptail << endl << endl;
...
//radial distribution histogram
  ...
  walker[ip] = rho*walker[it] + 48./(3.*vol)*w; //Pressure
  return;
}
...
{ 
  if(iNVET)
    { //Equilibration for MC measure
    for(int i = 0; i < 5000; i++)  Move();
    }

  else
  { //Equilibration for MD measure
    double appotemp = 0.; 
    ifstream EqConf;

    if(phase == "gas") appotemp = 0.95*temp/1.2;
    if(phase == "liquid") appotemp = 2.0*temp/1.1;
    if(phase == "solid")  appotemp = 1.54*temp/0.8;

    EqConf.open("config.fcc");
    double sumv[3] = {0.0, 0.0, 0.0};
    for (int i=0; i<npart; ++i)
    {
      vx[i] = rnd.Gauss(0.,sqrt(appotemp));
      vy[i] = rnd.Gauss(0.,sqrt(appotemp));
      vz[i] = rnd.Gauss(0.,sqrt(appotemp));
      sumv[0] += vx[i];
      sumv[1] += vy[i];
      sumv[2] += vz[i];
    }
    ...
    sumv2 /= (double)npart;
    fs = sqrt(3 * appotemp / sumv2);   // fs = velocity scale factor 
    for (int i=0; i<npart; ++i)
    {
      vx[i] *= fs;
      vy[i] *= fs;
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Press , Gofr , Gave;
   const int wd=12;
    ...
    Epot.open(path + phase + to_string(restart) + "_output_epot.dat",ios::app);
    Press.open(path + phase + to_string(restart) +"_output_press.dat",ios::app);
    Gofr.open(path + phase + to_string(restart) + "_output_gofr.dat",ios::app);
    Gave.open(path + phase + to_string(restart) + "_output_gave.dat",ios::app);

    if(iNVET ==  0) 
    {
    Ekin.open(path  + phase + to_string(restart) +"_output_ekin.dat",ios::app);
    Temp.open(path + phase + to_string(restart) +"_output_temp.dat",ios::app);
    Etot.open(path+ phase + to_string(restart) +"_output_etot.dat",ios::app);
    }
...
// Pressure
    Press << setw(wd) << iblk <<  setw(wd) << stima_pres << setw(wd) << glob_av[ip]/(double)iblk << setw(wd) << err_press << endl;

//g(r)
    for (int i=0; i<nbins; i++){
	    r = i*bin_size;
	    stima_gidir = blk_av[ig + i]/blk_norm/(rho*npart*(4./3.)*pi*(pow(r+bin_size,3)-pow(r,3))); //Radial istribution function
	    glob_av[ig + i] += stima_gidir;
	    glob_av2[ig + i] += stima_gidir*stima_gidir;
	    err_gdir=Error(glob_av[ig + i],glob_av2[ig + i],iblk);
	    Gofr << setw(wd) << iblk <<  setw(wd) << r <<  setw(wd)  << setw(wd) << glob_av[ig + i]/(double)iblk << setw(wd) << err_gdir << endl;
	    if (iblk==nblk)    Gave << setw(wd) << r <<  setw(wd)  << setw(wd) << glob_av[ig + i]/(double)iblk << setw(wd) << err_gdir << endl;
...
void ConfFinal(void)
{
  ...
  cout << "Print final configuration to files: " + path_out + phase + "_config.out and " + path_out + phase + "_velocity.out" << endl << endl;
  WriteConf.open(path_out + phase + "_config.out");
  WriteVelocity.open(path_out + phase + "_velocity.out");
  for (int i=0; i<npart; ++i)
  {
...
void ConfXYZ(int nconf){ //Write configuration in .xyz format
  ...
  WriteXYZ.open("frames/" + phase +"/" + phase + "_config_" + to_string(nconf) + ".xyz");
  WriteXYZ << npart << endl;
