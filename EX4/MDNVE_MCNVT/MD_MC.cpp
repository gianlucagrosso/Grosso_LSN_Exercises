...
//Read input informations
  ...
  path = "DATA/" + phase + "/";
  average = path + "averages/";
  equilibration = path + "equilibration/";
...
//Read initial configuration
  ...
  if(restart)
  {
    ReadConf.open(path + phase + "_config.out");
    ReadVelocity.open(path + phase + "_velocity.out");
    for (int i=0; i<npart; ++i) ReadVelocity >> vx[i] >> vy[i] >> vz[i];
  }
  else 
  {
    ReadConf.open(path + "config.in");
...
void Averages(int iblk) //Print results for current block
{
  ...
  if(iEQ == 1)
  {
    Epot.open(equilibration + phase + "_output_epot.dat",ios::app);
    Ekin.open(equilibration  + phase + "_output_ekin.dat",ios::app);
    Temp.open(equilibration + phase + "_output_temp.dat",ios::app);
    Etot.open(equilibration+ phase + "_output_etot.dat",ios::app);
    Press.open(equilibration + phase + "_output_press.dat",ios::app);
  }
  ...
  {
    Epot.open(average + phase + "_output_epot.dat",ios::app);
    Ekin.open(average  + phase + "_output_ekin.dat",ios::app);
    Temp.open(average + phase + "_output_temp.dat",ios::app);
    Etot.open(average+ phase + "_output_etot.dat",ios::app);
    Press.open(average + phase + "_output_press.dat",ios::app);
  }
...
void ConfFinal(void)
{
  ...
  cout << "Print final configuration to file: " + phase + "_config.out" << endl << endl;
  WriteConf.open(path + phase + "_config.out");
  WriteVelocity.open(path + phase + "_velocity.out");
  for (int i=0; i<npart; ++i)
  {
